...
function renderCustomTrendsChart(matchHistory, selectedMetrics) {
    ...
    if (selectedMetrics.includes('timePlayed')) {
        datasets.push({
            label: 'Time Played Per Game (min)',
            data: timePlayedArr,
            borderColor: '#16a085',
...
function aggregateByPeriod(matchHistory, groupBy) {
    ...
    // For timePlayed, average per group
    Object.values(groups).forEach(g => {
        if (g.count > 0) g.timePlayed = g.timePlayed / g.count;
    });
    return groups;
}
...
function renderCustomTrendsPerDayChart(matchHistory, selectedMetrics, groupBy) {
    ...
    const metricsMap = {
        ...
        timePlayed: { label: 'Time Played Per ' + groupBy.charAt(0).toUpperCase() + groupBy.slice(1) + ' (min)', color: '#16a085' },
